...
def plot_yoy_chart(df, iso, value_label):
    temp = df[df['iso'] == iso]
    fig, ax = plt.subplots(figsize=(6, 3))  # size

    width = 0.35
    x = range(len(temp))
    ax.bar([i - width/2 for i in x], temp['2024'], width=width, label='2024', color='#4B8BBE')
    ax.bar([i + width/2 for i in x], temp['2025'], width=width, label='2025', color='#306998')

    ax.set_xticks(x)
    ax.set_xticklabels(temp['category'], rotation=15, ha='right')
    ax.set_title(f"{value_label} - {iso}", fontsize=14)
    ax.set_ylabel(value_label)
    ax.set_xlabel("Account Type")
    ax.legend()
