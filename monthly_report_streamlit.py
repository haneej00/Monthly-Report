...
def classify_account_type(df):
    df = df.copy()
    df['account_category'] = df['accounttype'].apply(
        lambda x: 'New account' if str(x).strip() in ['New', 'Conversion'] else str(x).strip()
    )
    return df

df24 = classify_account_type(df24)
df25 = classify_account_type(df25)
